 ...
 * <p>日期：2018年07月10日 12:55</p>
 * <p>版权所有：广东联结网络技术有限公司 版权所有(C) 2016-2018</p>
 */
public final class ExcelExUtils {

    /**
     * 单例模式
     * 通过{@link ExcelExUtils#getInstance()}获取对象实例
     */
    private static volatile ExcelExUtils excelUtils;

    private ExcelExUtils() {
    }

    /**
     * 双检锁保证绝对线程安全
     */
    public static ExcelExUtils getInstance() {
        if (null == excelUtils) {
            synchronized (ExcelExUtils.class) {
                if (null == excelUtils) {
                    excelUtils = new ExcelExUtils();
                }
            }
        }
        return excelUtils;
    ...
    private <T> List<T> readExcel2ObjectsHandler(Workbook workbook, Class<T> clazz, int offsetLine,
        ...
        Map<Integer, ExcelHeader> maps = ExUtils.getHeaderMap(row, clazz);
        if (maps == null || maps.size() <= 0) {
            maps = ExUtils.getHeaderMap( sheet.getRow((offsetLine=offsetLine+1)), clazz);
            if(maps==null || maps.size()<=0)
                throw new BizException("Excel 标题数据格式错误.");
        }
        ...
        for (int i = offsetLine + 1; i <= maxLine; i++) {
            ...
            for (Cell cell : row) {
                ...
                if (null == header)
                    continue;
                String val = ExUtils.getCellValue(cell);
                Object value;
                ...
                if (null != header.getReadConverter() &&
                    ...
                    // 默认转换
                    value = ExUtils.str2TargetClass(val, header.getFiledClazz());
                }
                ExUtils.copyProperty(obj, filed, value);
            }
            list.add(obj);
        }
        return list;
    ...
    private void generateSheet(int sheetIndex, List<?> data, Map<String, String> extendMap, Class clazz,
        ...
        List<ExcelHeader> headers = ExUtils.getHeaderList(clazz);
        if (isWriteHeader) {
            // 写标题
        ...
        for (Object object : data) {
            ...
            for (ExcelHeader header : headers) {
                SheetTemplateHandler.createCell(template, ExUtils.getProperty(object, header.getFiled(),
            ...
            for (Object object : entry.getValue()) {
                ...
                for (ExcelHeader header : headers) {
                    SheetTemplateHandler.createCell(template,
                            ExUtils.getProperty(object, header.getFiled(), header.getWriteConverter()),
                            entry.getKey()
                    );
                }
            }
    ...
    private void generateSheet(Workbook workbook, List<?> data, Class clazz,
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            for (int j = 0; j < headers.size(); j++) {
                row.createCell(j).setCellValue(ExUtils.getProperty(_data,
