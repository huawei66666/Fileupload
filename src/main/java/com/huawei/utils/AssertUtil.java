...
//			throw new IllegalStateException( message );
    ...
    /**
     * 限定参数的长度不能超过指定值，否则抛出异常。 当 s 不等于null（如果为null忽略校验），并且长度超出maxLength才抛出异常。
     *
     * @param str       - 要判断的字符串
     * @param maxLength - 指定长度
     * @param message   - 出错时的异常错误信息
     * @throws BizException - 出错时抛出异常
     */
    public static void assertNotNullMaxLength(String str, int maxLength, String message) {
     ...
     * @throws BizException 出错时抛出异常
     */
    public static void assertEqual(String a, String b, String errorMessage) {
        isNotNull(a);
        isNotNull(b);
        if (!a.equals(b)) {
            throw new BizException(errorMessage);
